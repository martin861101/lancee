...
- Default boundary restricts external tools to `Browser, Utilities` categories.
- Always-active built-in **Lancee** service (nine built-in tools).
- Optional external MCP services with workspace-scoped activation.
- Bearer access state machine: `available → pending or approved → activated`.
- Server-side auto-approval when `MCP_API_TOKEN` is configured; otherwise
  pending for admin grant.
- Catalog discovery and tool invocation are live server-to-server calls.
- Bearer tokens never enter the browser.
- Service status (live / unreachable), credential modes, and per-service toggles.
- **Approve & run** control before invoking any tool.
- Every invocation audited and resolved to the live runtime at the server
  boundary.
- Basebox integrated as an authenticated MCP Streamable HTTP server with live
  tool-catalog discovery.
- Missing or rejected credentials shown honestly, never replaced with fallback
  tools.

## PDF Studio

- Styled PDF document generator.
...
- Official Anthropic `mcp-server-dev` Claude plugin vendored for building
  remote MCP servers, MCP apps, and MCPB packages.
- Invocable from a Claude environment with
  `/mcp-server-dev:build-mcp-server` after registering the plugin directory.

...
```sh
npx shadcn@latest add @canvas-ui/particle-scroll-react
```
...
const props = defineProps<ParticleScrollOptions>();
...
<script lang="ts">
  ...
  interface Props extends ParticleScrollOptions {
    class?: string;
...
| --- | --- | --- | --- |
| point | number | 0.68 | Viewport fraction of the formation line. Content assembles as it scrolls up past this line and dissolves back below it. |
| band | number | 420 | Height in CSS pixels of the transition band where particles progressively reassemble. |
| density | number | 2 | Grain spacing in CSS pixels. Smaller values mean finer, denser sand. |
| size | number | 1.25 | Size of fully scattered dust grains in CSS pixels. Grains grow to cover their cell as they land. |
| spread | number | 220 | Maximum distance in CSS pixels particles scatter from their home position. |
| gravity | number | 0.35 | Downward bias of the scattered cloud (-1 to 1), like sand settling. Negative values lift it. |
| drift | number | 0.7 | Idle float speed of scattered particles (0 to 1). 0 freezes the cloud. |
| swirl | number | 60 | Sideways arc in CSS pixels particles take while flying home. |
| stagger | number | 0.7 | Per-particle randomness of reassembly timing (0 to 1). |
| fade | number | 0.85 | Opacity of fully scattered particles (0 to 1). |
| settle | number | 1.2 | Seconds a row of dust takes to condense into the page once the reveal reaches it. |
| smoothing | number | 0.6 | Seconds the damped scroll takes to catch up with the real scroll. Higher feels more fluid. |
